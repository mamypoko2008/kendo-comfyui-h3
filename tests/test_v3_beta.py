...
class V3BetaReadinessTest(unittest.TestCase):
    ...
    def test_status_exposes_partial_readiness_and_beta_version(self):
        ...
        with tempfile.TemporaryDirectory() as folder, \
            ...
            try:
                ...
                self.assertEqual(result['version'], '3.0.0-beta.9')
            finally:
                server.shutdown()
                server.server_close()
    ...
    def test_entrypoint_restores_comfy_before_linking_upscalers(self):
        ...
        link_target = script.index('for node_name in ComfyUI-SeedVR2_VideoUpscaler')
        self.assertLess(restore, link_target)
        self.assertIn('[[ ! -f "$comfyui_dir/main.py" ]]', script)
        self.assertIn('if [[ -L "$old_flash" ]]', script)
        self.assertIn('if [[ -L "$old_rtx" ]]', script)
        dockerfile = (Path(__file__).resolve().parents[1] / 'Dockerfile.v3-beta').read_text()
        self.assertNotIn('nvidia-vfx', dockerfile)
        self.assertNotIn('Nvidia_RTX_Nodes_ComfyUI', dockerfile)
