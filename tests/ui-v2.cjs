...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>localStorage.setItem('kendo-h3-v2-history',JSON.stringify([{url:'/api/comfy/view?filename=old.mp4',prompt:'PROMPT MUST NOT APPEAR'}])));
  ...
  assert.deepEqual(errors,[]);console.log('UI passed: prompt-free history, resizable controls, slider 5-20s, resolutions through 2MP, steps 10, readiness gate, light/dark persistence, limits 9/1/3, disabled video omission, retained file, mobile width');
