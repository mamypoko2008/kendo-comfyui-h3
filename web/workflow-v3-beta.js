...
(function (root) {
  ...
  function buildWorkflow({
    prompt, ratio, megapixels, duration, steps,
    seed = Math.floor(Math.random() * 0x100000000),
    images = [], videos = [], audios = [], videoAudio = false,
    realismEnabled = true, realismWeight = 0.75
  }) {
    ...
    if (typeof realismEnabled !== 'boolean' || !Number.isFinite(realismWeight) || realismWeight < 0 || realismWeight > 2) throw new Error('Invalid LoRA settings');
    ...
    const triggerPattern = new RegExp('(^|\\s)' + REALISM_TRIGGER + '(?=\\s|,|$)', 'i');
    const conditionedPrompt = realismEnabled && !triggerPattern.test(prompt)
      ? REALISM_TRIGGER + ', ' + prompt.trim()
      : prompt.trim();
  ...
  if(typeof module !== 'undefined') module.exports={buildWorkflow,REALISM_LORA,REALISM_TRIGGER};
  else root.KendoWorkflow={buildWorkflow,REALISM_LORA,REALISM_TRIGGER};
})(globalThis);
